    ...
    <div className="relative w-full px-5 md:px-10 lg:px-20">
      ...
      <div className="grid grid-cols-1 gap-y-6 gap-4 lg:gap-12 mb-13  md:mb-16 lg:mb-24">
        {tabContent.length > 0 ? (
          tabContent.slice(0, 1).map((data) => (
            ...
            >
              ...
              <div className="blog-content flex flex-col place-items-start gap-1 pt-3">
                ...
                <p className="text-sm  md:text-xl lg:text-lg text-gray-800  leading-[140%] w-full pr-2 tracking-[-1.1%]" dangerouslySetInnerHTML={{__html: data.description}}>
          ...
          <p className="text-black-400 text-center text-xl mt-10 ">No content available for {activeTab}.</p>
        )}
      </div>
    </div>
  );
