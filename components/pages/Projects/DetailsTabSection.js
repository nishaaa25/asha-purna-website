...
export default function DetailsTabSection({ 
  ...
  const hasGalleryImages = () => {
    ...
    if (projectVideoGalleryData && projectVideoGalleryData.length > 0) return true;
    
    // Check for gallery categories with actual images
    if (projectGalleryData?.gallery_names?.length > 0) {
      return projectGalleryData.gallery_names.some(category => 
  ...
  if (project?.master_plan_image || 
      (projectFloorsData && projectFloorsData.project_floor_plans && projectFloorsData.project_floor_plans.length > 0)) {
  ...
  return (
    <div ref={tabSectionRef} className="w-full relative pt-15 pb-10 md:pb-20 lg:pb-[100px]">
      ...
      {activeTab === "overview" && (
        <div className="flex-center w-full px-[22px] relative flex-col text-center pt-3 md:pt-6 lg:pt-10 gap-7">
          ...
          <div 
            ...
            dangerouslySetInnerHTML={{ __html: project?.description || project?.overview || "" }}
          />
          ...
          <div className="w-full bg-white rounded-lg ">
            <div className="relative w-full md:w-10/12 lg:w-8/12 mx-auto">
              <SectionHeader
                ...
                desc={project?.plan_description || "Choose from a range of plot sizes tailored for industries, logistics, and agri-based enterprises. Infrastructure and utilities are ready, ensuring ease of setup."}
              />
            </div>
